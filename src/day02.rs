...
//! "Destination reached. Current Year: 1518. Current Location:
...
//! Another voice responds, "Actually, we've been working on a new kind of suit that would let
//! him fit through tight spaces like that. But, I heard that a few days ago,
...
//! could cause if you were discovered - and use your fancy wrist device to
...
fn part1(input: &str) -> i32 {
    let mut count2 = 0;
    let mut count3 = 0;
    input.trim().lines().map(|s| Id(s.trim())).for_each(|id| {
        let (has_two, has_three) = id.score();
        if has_three {
            count3 += 1;
        }
...
/// Find the IDs which differ with exactly one character
/// and print the common characters.
fn part2(input: &str) -> String {
    let ids = input.lines().map(|s| s.trim()).collect::<Vec<_>>();

    for id in &ids {
        let other = ids.iter().find(|other| {
            other
                .chars()
                .zip(id.chars())
                .filter(|(c1, c2)| c1 != c2)
...
impl Id<'_> {
    fn score(&self) -> (bool, bool) {
        let cs = self.0.chars();
        let (mut has_two, mut has_three) = (false, false);
        cs.clone().for_each(|c| {
            let count = cs.clone().filter(|&c2| c2 == c).count();
            if count == 3 {
                has_three = true;
            } else if count == 2 {
                has_two = true;
